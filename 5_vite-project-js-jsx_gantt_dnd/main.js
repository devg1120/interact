...
import { getData, formatXML } from '/data';
...
const position2 = { x: 0, y: 0 };
    ...
    .draggable({
      ...
      listeners: {
        ...
        move: (event) => {
          ...
          //event.target.style.transform = translate('position.x + 'px', '0px')
          //event.target.style.transform = "translate(0px', 0px')"
	//event.target.setAttribute("cx", 100 );
    ...
    .on('resizemove', (event) => {
      position.x += event.deltaRect.left
      //position2.x += event.rect.left
      position2.x += event.target.dx;
      console.log(position.x)
      ...
      if (position.x == 0) {
         // event.target.style.left = position.x + 'px'
          ...
          //let old = event.target.style.left
          //event.target.style.left = position.x + 'px'
          //event.target.style.x =  event.dx +   position.x
          event.target.style.transform =  `translate3D(${position.x}px, 0, 0)`

          //event.target.style.x = (parseInt(event.target.style.left) + position.x) + 'px'
          //event.target.style.x = (parseInt(event.target.style.x) + position.x) + 'px'
          //event.target.style.x = (parseInt(event.target.style.x) + position.x) + 'px'
          //event.target.style.x = 30 + 'px'
          //event.target.style.left = position2.x + 'px'
          //event.target.style.x = (position2.x + position.x) + 'px'

      }
      //event.target.style.x = position.x + 'px'
